...
class PsychUISlider extends FlxSpriteGroup {
	...
	public dynamic function onChange(value:Float):Void {}
	...
	override function update(elapsed:Float) {
		...
		if (FlxG.mouse.justMoved || FlxG.mouse.justPressed || forceNextUpdate) {
			...
			if (movingHandle) {
				...
				if (lastValue != value) {
					onChange(FlxMath.roundDecimal(value, decimals));
	...
	function _updatePositions() {
		minText.x = bar.x - minText.width * 0.5;
		maxText.x = bar.x + bar.width - maxText.width * 0.5;
		valueText.x = bar.x + bar.width * 0.5 - valueText.width * 0.5;

		labelText.x = bar.x + bar.width * 0.5 - labelText.width * 0.5;
		if (label.length > 0) bar.y = labelText.y + 24;
		
		minText.y = maxText.y = valueText.y = bar.y + 12;

		_updateHandleX();
		handle.y = bar.y + bar.height * 0.5 - handle.height * 0.5;
	}

	function _updateHandleX()
		handle.x = bar.x - handle.width * 0.5 + FlxMath.remapToRange(FlxMath.roundDecimal(value, decimals), min, max, 0, bar.width);
