...
class PsychUIRadioGroup extends FlxSpriteGroup {
	...
	override function update(elapsed:Float) {
		...
		if (arrowDown.exists && arrowDown.active) {
			arrowDown.x = x;
			arrowDown.y = baseY;
			hasArrowDown = true;
		}

		if (arrowUp.exists && arrowUp.active) {
			arrowUp.x = x;
			arrowUp.y = baseY;
			hasArrowUp = true;
		...
		if (FlxG.mouse.justPressed) {
			...
			if (hasArrowDown && arrowDown.animation.curAnim != null && arrowDown.animation.curAnim.name != 'normal') arrowDown.animation.play('normal');
		}
	}
	...
	override function draw() {
		...
		if (arrowUp.exists && arrowUp.active) arrowUp.draw();
		if (arrowDown.exists && arrowDown.active) arrowDown.draw();
	}
	...
	function set_textWidth(v:Int) {
		textWidth = v;
		for (radio in radios) radio.text.fieldWidth = textWidth;
	...
	override function set_cameras(v:Array<FlxCamera>) {
		if (arrowUp.exists) arrowUp.cameras = v;
		if (arrowDown.exists) arrowDown.cameras = v;
		return super.set_cameras(v);
	}

	override function set_camera(v:FlxCamera) {
		if (arrowUp.exists) arrowUp.camera = v;
		if (arrowDown.exists) arrowDown.camera = v;
