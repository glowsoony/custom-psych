...
class NativeAudioSource
{
	...
	// Event Handlers
	private function streamTimer_onRun():Void refillBuffers();
	...
	public function setCurrentTime(value:Float):Float {
		if (handle != null) {
			if (stream) {
				...
				var totalOffset:Int = Std.int(dataLength * ratio);
		...
		if (playing) {
			...
			var timeRemaining:Int = Std.int((getLength() - value) / getPitch());
	...
	public function setLoops(value:Int):Int return loops = value;
