...
class PsychUINumericStepper extends PsychUIInputText {
	...
	public dynamic function onValueChange():Void {}
	...
	override function update(elapsed:Float) {
		...
		if (FlxG.mouse.justPressed) {
			if (buttonPlus.exists && FlxG.mouse.overlaps(buttonPlus, camera)) {
				buttonPlus.animation.play('pressed');
				value += step;
				_internalOnChange();
			} else if (buttonMinus.exists && FlxG.mouse.overlaps(buttonMinus, camera)) {
				buttonMinus.animation.play('pressed');
				value -= step;
				_internalOnChange();
			}
		} else if(FlxG.mouse.released) {
			if (buttonPlus.exists && buttonPlus.animation.curAnim != null && buttonPlus.animation.curAnim.name != 'normal')
				buttonPlus.animation.play('normal');
			if (buttonMinus.exists && buttonMinus.animation.curAnim != null && buttonMinus.animation.curAnim.name != 'normal')
				buttonMinus.animation.play('normal');
		}
	}
