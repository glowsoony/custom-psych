...
class Native
{
	...
	public static function fixScaling():Void
	{
		...
		if (display != null)
		{
			...
			@:privateAccess Application.current.window.width = Std.int(flixel.FlxG.initialWidth * dpiScale);
			@:privateAccess Application.current.window.height = Std.int(flixel.FlxG.initialHeight * dpiScale);
